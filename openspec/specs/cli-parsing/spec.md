...
Define the command-line interface for git-paw using clap v4. Declares all subcommands (`start`, `stop`, `purge`, `status`, `list-clis`, `add-cli`, `remove-cli`), their flags, and argument validation. When no subcommand is given, defaults to `start`. This includes the `git paw start --force` flag that suppresses the uncommitted-spec validation warning (logging force usage to stderr for audit and propagating `git status` failures as `PawError::GitError`), and the guarantee that `git paw start --supervisor --specs <names>` passes the resolved `--specs` value (not just its presence) to the supervisor launch path so it launches worktrees only for the named subset — pinned by a regression matrix across every selection-flag/supervisor-flag combination and required to behave identically across all spec backends (OpenSpec, Markdown, Spec Kit).
## Requirements
