...
- [x] Pass 3b (naming refinement, user review) — `mcp`→`mcp-server` (bare name ≠ its namespace) and `spec-backends`→`spec-providers`; dissolved `governance-`→`core-` (core-governance-config, core-opsx-role-gating) and `quality-`→ split (`core-selftest` + `supervisor-learnings`, a runtime feature). **12 namespaces / 40 caps / 729 reqs**; docs index regrouped; mdbook + anti-drift + validate + `cargo test --no-run` green
