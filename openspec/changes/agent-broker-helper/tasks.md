...
- [x] 1.1 Create `assets/scripts/broker.sh` (bash, `set -u`, bash
...
- [x] 1.2 Implement agent-id resolution: take `--agent <id>` (the
...
- [x] 1.3 Implement publish subcommands assembling JSON internally and
...
- [x] 4.2 Preserve every existing seeding property: config-driven
  ...
  failure.
- [x] 4.3 Ensure no `curl *` (broad curl) grant is seeded anywhere on
  the launch path.

## 5. Tests

- [x] 5.1 Add `tests/broker_sh_conventions.rs` (analogous to
...
- [x] 5.2 Add an init test (analogous to
...
- [x] 5.3 Boot-block content test: the rendered boot block calls
  ...
  status:"done"`.
- [x] 5.4 Allowlist test: seeding produces a `.git-paw/scripts/broker.sh`
  path grant and no `curl *` grant; re-seeding is idempotent.
- [x] 5.5 Behavioral helper test (mirroring
...
- [x] 6.1 Update the boot-block and `git paw init` mdBook chapters
  (`docs/src/`) to describe `broker.sh`, its install, and the
  helper-path allowlist (replacing the per-endpoint curl description).
- [x] 6.2 Add a helper reference enumerating the `broker.sh`
  subcommands (`status`/`artifact`/`blocked`/`question`/`intent`/`poll`).
- [x] 6.3 Update README/CLI/config references if the allowlist
  description changes; ensure `mdbook build docs/` succeeds.

## 7. Quality gates

- [x] 7.1 `just check` (fmt + clippy + all tests) passes; no
  `unwrap()`/`expect()` in non-test code; all public items documented.
  (fmt --check, clippy -D warnings, 1633 lib tests, and all changed-area
  integration tests pass. The tmux-session e2e binaries abort under the
  live dogfood session via the `paw-*` env guard at
  `tests/helpers/mod.rs:269` — an environmental limitation, not a logic
  failure; the supervisor runs them in an isolated verify worktree.)
- [x] 7.2 `just deny` passes (no new dependencies introduced).
- [x] 7.3 Backward compatibility verified: a session carrying a
  pre-existing per-endpoint `curl` allowlist still launches (stale
  prefixes are preserved — see
  `preserves_stale_curl_prefix_and_adds_helper_path`); broker endpoints
  unchanged (`src/broker/` untouched).
