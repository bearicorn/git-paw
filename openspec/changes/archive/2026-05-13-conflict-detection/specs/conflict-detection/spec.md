...
- `agent.question` messages emitted to the supervisor inbox SHALL set `agent_id = "supervisor"` (the recipient), `payload.from = "supervisor"` (matching v0.4 supervisor-originated convention), and SHALL include `[conflict-detector]` as a token in the question text.
