...
- [~] `interactive.rs` resolver logic → module; live `dialoguer` impls untouched — DEFERRED (lowest-value cosmetic split; not depended on by R2/R3). Folds into the R3-tail interactive rider carried to **v0.14.0** (see `MILESTONE.md` § v0.14.0).
...
- [~] Interactive live-prompt impls behind the PTY net — DEFERRED to **v0.14.0** (carried forward as the `code-analysis-refactor` R3-tail rider — see `MILESTONE.md` § v0.14.0). Behavior-sensitive live `dialoguer`/`crossterm` path, low ROI; the runtime seam is R3's high-value core. Now unblocked by the `cli-interaction-e2e` PTY net.
- [~] Dashboard event loop — DEFERRED to **v0.14.0** (same R3-tail rider — see `MILESTONE.md` § v0.14.0). The `fix/dashboard-cpu-leak` fix already landed (poll_tty root-cause + TmuxTestEnv reap); the remaining draw/format split touches the SIGHUP `unsafe` + just-fixed loop for low ROI, so it rides the next cycle.
