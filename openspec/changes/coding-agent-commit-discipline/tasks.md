...
- [x] 1.1 Add a positive **stand-by-after-commit** protocol. Strengthen the existing
...
- [x] 1.2 Add **releasable-unit + amend-fixups** guidance to the `### Commit cadence`
...
- [x] 1.3 **De-opinionate the commit-format prose**: replace the "Use the project's
...
- [x] 2.1 In the supervisor skill's commit-cadence / verification guidance, add a note
...
- [x] 3.1 Add a skill-content test (new `tests/commit_discipline_skill_content.rs`, or
...
- [x] 3.2 Add skill-content tests asserting the **releasable-unit + amend** prose: each
...
- [x] 3.3 Add a skill-content test asserting the **commit-format deferral**: the
...
- [x] 3.4 Add a skill-content test asserting the **supervisor cross-reference**: the
...
- [x] 3.5 Audit existing skill-content / lang-agnostic-audit tests
...
- [x] 4.1 Update the skill/agent docs that describe the post-commit protocol and commit
...
- [x] 5.1 `just check` (fmt + clippy + all tests) passes.
- [x] 5.2 `just deny` passes (no dependency change expected — skill-prose-only).
- [x] 5.3 `openspec validate coding-agent-commit-discipline --strict` passes.
- [x] 5.4 Confirm backward compatibility: no code-path or config change; the skill renders
